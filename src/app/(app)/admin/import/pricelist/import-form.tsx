...
import { importPricelistStock, type ImportState } from "./actions";
import { Card } from "@/components/ui";

const initialState: ImportState = {};

export function ImportForm() {
  const [state, formAction, isPending] = useActionState(importPricelistStock, initialState);
      ...
      {state.summary && (
        <Card className="p-5">
          ...
          <dl className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
            ...
            <div>
              <dt className="text-slate-500">Prices set</dt>
              <dd className="text-lg font-semibold text-slate-800">{state.summary.priceEntriesSet}</dd>
            </div>
            <div>
              <dt className="text-slate-500">Stock entries set</dt>
              <dd className="text-lg font-semibold text-slate-800">{state.summary.stockEntriesSet}</dd>
            </div>
          </dl>
